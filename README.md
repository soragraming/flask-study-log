...
- **学習目標**：ポートフォリオを自分の言葉で説明できるようになる！
- **学習方針**：1時間：基礎/ 1時間：ポートフォリオ解説
...
### [特殊メソッド:**call**](03_class/base5_special.py#41)

- インスタンスを関数みたいに()で呼び出せる
- 通常、クラスから作成したインスタンスは'instance.method()'と呼び出しますが、'**call**'を定義すると'instance'だけで実行されるようになります。
- **用途**：設定を保持したままあとで何度も実行したい処理などに使われる。

## 📅 2026/03/06

### [特殊メソッド](03_class/base5_special.py)

- **init**,インスタンス作成時,初期設定
- **str**,print() や str() に渡された時,ユーザー向けの「読みやすい」説明を返す
- **len**,len() に渡された時,そのオブジェクトの「長さ」を定義する

以下どちらもオブジェクトを文字列で表現するための特殊メソッドだが、用途と動作が異なる。

- **repr**開発者向けの詳細な情報を返す。(デバックや開発中の詳細情報を確認するとき)
- **str**ユーザー向けのわかりやすい文字列を返す（一般ユーザーへの表示）

## 📅 2026/03/05

### [メソッドの3分類](03_class/base4_method.py)

1. **インスタンスメソッド (`self`)**: 個別の実体（車）を操作する。
2. **クラスメソッド (`@classmethod`, `cls`)**: 設計図（工場）全体の管理や、新しいインスタンスの生成に使用。
3. **スタティックメソッド (`@staticmethod`)**: クラスやインスタンスに依存しない「独立した道具・ルール」として定義。

- **学び**: 複雑な条件分岐は `is_valid_xxx` のように肯定文でルール化し、スタティックメソッドに切り出すとデバッグが容易になる。

### [インスタンスメソッド](03_class/base4_method.py)

- クラスのインスタンス(オブジェクト)に関連づけられたメソッド。
- 第一引数として自動的にselfを受けとる(selfはインズタンス自身を参照)
- インスタンス変数にアクセスできる
- インスタンスを通じて呼び出す

### [クラスメソッド](03_class/base4_method.py)

- インスタンスではなくクラス自体に関連付けられたメソッド
- @classmethodデコレータで定義
- クラス変数にアクセスできる
- インスタンスからもクラスからも呼び出せる

### [スタティックメソッド](03_class/base4_method.py)

- クラスやインスタンスの状態に依存しないユーティリティーメソッド
- @staticmethodデコレータで定義
- 自動的な第一引数なし（selfもclsも受け取らない）
- インスタンス変数やクラス変数にアクセスできない(引数として渡さない限り)
- インスタンスからもクラスからも呼び出せる

## 📅 2026/03/04

### [class](03_class/base2_class.py)

**クラス変数**：クラス全体で共有される変数
**インスタンス変数**各インスタンス固有の変数
→大規模システム等で基本設定(クラス変数)はあるけど、特定のユーザーだけを特別設定にする等のコードに活用

- self.変数名 = 値で新しいインスタンス変数が作成される
- インスタンス変数がある場合、それがクラス変数より優先される
- メモリIDで実際の参照先を確認できる

## 📅 2026/03/03

学習方針の転換：これまではUdemy動画講座にてPython基礎学習を一通り通ることを進めてきたが、範囲が広く時間がかかるため、ポートフォリオを自分の言葉で説明できるようになることを優先すべく、関連する講座項目だけに絞るこことした。

### class

- **インスタンス**: クラス(設計図)を実体化したもの。属性とメソッドを持つ。
- **init**: インスタンス化の瞬間に自動実行される特殊メソッド。個別の「初期設定」を行う。
- **self**: インスタンス自身を指す。これがあるから、複数の車（インスタンス）を作ってもデータが混ざらない。
- **クラス変数 vs インスタンス変数**: 設計図共通のデータか、個別のデータか。インスタンス変数はクラス変数を「隠す（上書きする）」。

## 📅 2026/03/02

### 単語の再確認

クロージャ:「設定（変数）」をカバンに詰め込んで持ち運ぶ関数
コールバック関数:「終わったらこれやっといて！」と後回しにするための関数
フィボナッチ:全員一気に呼ぶのではなく、一人ずつ「次の方どうぞ」と呼び出す受付（メモリに優しい）
ジェネレータ:前の2人を足すと自分になる」というルールの数字の列。再帰関数の練習によく使われる

### 内包表記

リスト、辞書、集合、ジェネレータなどのデータ構造を簡潔かつ効率的に作成するための構文。通常のループと条件分岐を一行でかけるため、コードが簡潔になり読みやすくなる。

### 再帰

関数が自分自身を呼び出すプログラミング手法。（わかりにくいため、あまり推奨されない）

### 高階関数

他の関数を引数として受け取ったり、関数を戻り値として返したりする機能

### lambda式

ラムダ関数は、名前を持たない小さな匿名関数で、簡単な関数を定義できる。
